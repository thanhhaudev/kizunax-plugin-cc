...
- If the raw arguments already include `--provider <name>`, launch without inserting a second flag:
  ```typescript
  Bash({
    command: `[ -f "${CLAUDE_PLUGIN_ROOT}/bin/kizunax" ] || { echo "Binary missing — run /kizunax:setup to build it."; exit 1; }; ${CLAUDE_PLUGIN_ROOT}/bin/kizunax review $ARGUMENTS`,
...
- Otherwise, prepend the routed choice from Step 1:
  ```typescript
  Bash({
    command: `[ -f "${CLAUDE_PLUGIN_ROOT}/bin/kizunax" ] || { echo "Binary missing — run /kizunax:setup to build it."; exit 1; }; ${CLAUDE_PLUGIN_ROOT}/bin/kizunax review --provider <chosen> $ARGUMENTS`,
    description: "Kizunax review",
    run_in_background: true
  })
  ```
